...
3. Fork this repository, then edit the file in your fork. GitHub's pencil button works; no software is required.
4. Change translation values only. Do not change, add, remove, or reorder source keys.
5. In blank files, leave `__SECTION_...`, `__COMMENT_...`, and number-only rows blank.
6. Do not add key bindings, numbers, rich-text tags, or selection arrows to blank files. Those are restored when the reviewed translation is prepared for the game.
7. Use natural human translation rather than unreviewed machine output.
8. Open a pull request titled `Translation: Full language name` and describe which sections are complete.
